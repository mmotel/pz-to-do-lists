	...
	io.sockets.on('connection', function (client) {
		'use strict';
			...
			client.on('editTask', function (editedTask){
				...
				Data.updateTask(editedTask, function (result){
					if(result === 1){
						Data.findAllTasks(editedTask.listid, function (items){
							Data.findList(editedTask.listid, function (list){
			...
			client.on('rmTask', function (rmedTask){
				...
				Data.removeTask(rmedTask.id, function (result){
					console.log(result);
					if(result === 1){
						Data.findAllTasks(rmedTask.listid, function (items){
							Data.findList(rmedTask.listid, function (list){
			...
			client.on('doneTask', function (doneTask){
				...
				Data.makeTaskDone(doneTask.id, function (result){
					if(result === 1){
						Data.findAllTasks(doneTask.listid, function (items){
							Data.findList(doneTask.listid, function (list){
