module.exports = function (Data){
	...
	var Task = function(id, fbid, listid, groupid, name, descr, deadline, executor, date){
		this.id = id;
		this.fbid = fbid;
		this.listid = listid;
		this.groupid = groupid;
		...
		var statusChange = {
			...
			"status": "new",
			"date": date,
			"comment": "Zadanie zostało utworzone."
		};
	...
	var Group = function(id, owner, name, descr, perms){
		this.id = id;
		this.owner = owner;
		this.name = name;
		this.descr = descr;
	...
	var AddTask = function(newTask, callback){
		Data.insertData("appTasks", 
			new Task(genId(), newTask.fbid, newTask.listid, newTask.groupid, newTask.name, newTask.descr, 
				newTask.deadline, newTask.executor, newTask.date), callback);
	};
	...
	var UpdateTask = function(newTask, callback){
		Data.updateData("appTasks", {"id": newTask.id}, 
			{ $set: {"name": newTask.name, "descr": newTask.descr, "deadline": newTask.deadline,
				"executor": newTask.executor} }, callback);
	};
	...
	var MakeTaskDone = function(data, callback){
		Data.updateData("appTasks", {"id": data.id}, { $set: {"status": data.status} }, function (err, result){
			if(err || result === 0){ callback(err, undefined); }
			else{
				var statusChange = {
					"fbid": data.fbid,
					"status": data.status,
				...
				Data.updateData("appTasks", {"id": data.id}, 
					{ $addToSet: { "statusChanges": { $each: [ statusChange ] } } }, callback);
			}
		});
	};

	var CreateGroup = function (newGroup, callback){
		Data.insertData("appGroups", 
			new Group(genId(), newGroup.owner, newGroup.name, newGroup.descr, newGroup.perms), callback);
	};
	...
	var UpdateGroup = function (newGroup, callback){
		Data.updateData("appGroups", {"id": newGroup.id},
			{ $set: {"name": newGroup.name, "descr": newGroup.descr, "perms": newGroup.perms} }, callback);
	};
	...
	var GetIncomingTasks = function(fbid, callback){
		FindAllLists(fbid, function (err, lists){
			if (err){ callback(err, undefined); }
			else{
				var listsIds = [];
				for(var i =0; i < lists.length; i++){
					listsIds.push(lists[i].id);
				}
				Data.findAllData("appTasks", 
					{"listid": { $in: listsIds }, "status": { $in: ["new", "progres"] }, "executor": fbid }, 
					function (err, tasks){
						if(err){ callback(err, undefined); }
						else{
							var d = new Date();
							var term = new Date(d.getFullYear(), d.getMonth(), d.getUTCDate()+7);

							var incomingTasks = [];

							for(var i =0; i < tasks.length; i++){
								var tdate = new Date(tasks[i].deadline.year, tasks[i].deadline.month -1, 
									tasks[i].deadline.day);
								if(tdate <= term){
									incomingTasks.push(tasks[i]);
								}
							}
							callback(undefined, incomingTasks);
						}
				});
			}
		});
